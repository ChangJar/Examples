...
int AesTest(char* fileIn, char* fileOut, byte* key, char* size)
{
    ...
    output = malloc(AES_BLOCK_SIZE * numBlocks);   /* sets size of output */
    ...
    if (choice == 'd') {
        ...
        for (i = sizeof(salt); i < AES_BLOCK_SIZE + sizeof(salt); i++) {
        ...
        length -= (AES_BLOCK_SIZE + sizeof(salt));
        for (i = 0; i < length; i++) {
            /* shifts message over an encryption block: ignores iv on message*/
            input[i] = input[i + (AES_BLOCK_SIZE + sizeof(salt))];
        }
...
int main(int argc, char** argv)
{
    ...
    if (argc != 5) {
        ...
        ret = AesTest(argv[3], argv[4], key, argv[2]);
    }
